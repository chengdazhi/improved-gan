...
gen_layers = [nn.batch_norm(ll.DenseLayer(ll.InputLayer(shape=(args.batch_size, 100), input_var=noise),
                                           num_units=512, W=Normal(0.05), nonlinearity=T.nnet.relu))] # Input layer for z1
gen_layer_noise = gen_layers[-1]
gen_layers.append(nn.batch_norm(ll.DenseLayer(ll.InputLayer(shape=(args.batch_size, 10), input_var=y_1hot),
                                               num_units=512, W=Normal(0.05), nonlinearity=T.nnet.relu))) # Input layer for labels
...
disc_layer_feature = disc_layers[-1]
disc_layers.append(nn.batch_norm(ll.DenseLayer(ll.InputLayer(shape=(None, 10), input_var=y_1hot),
                                               num_units=192, W=Normal(0.05), nonlinearity=T.nnet.relu))) # Input layer for labels
disc_layer_y = disc_layers[-1]
disc_layers.append(ll.ConcatLayer([disc_layer_feature,disc_layer_y],axis=1))
...
temp = ll.get_output(disc_layers[-1], {disc_layer_x: x_lab}, deterministic=False, init=True)
init_updates = [u for l in gen_layers+disc_layers for u in getattr(l,'init_updates',[])]

output_before_softmax_lab = ll.get_output(disc_layers[-1], {disc_layer_x: x_lab}, deterministic=False)
output_before_softmax_unl = ll.get_output(disc_layers[-1], {disc_layer_x: x_unl}, deterministic=False)
output_before_softmax_gen = ll.get_output(disc_layers[-1], {disc_layer_x: gen_dat}, deterministic=False)
...
# test error
output_before_softmax = ll.get_output(disc_layers[-1], {disc_layer_x: x_lab}, deterministic=True)
...
init_param = th.function(inputs=[x_lab,y_1hot], outputs=None, updates=init_updates) # data based initialization
...
samplefun = th.function(inputs=[y_1hot],outputs=gen_dat)

# Theano functions for training the gen net
output_unl = ll.get_output(disc_layers[-2], {disc_layer_x: x_unl}, deterministic=False)
output_gen = ll.get_output(disc_layers[-2], {disc_layer_x: gen_dat}, deterministic=False)
...
train_batch_gen = th.function(inputs=[x_unl, y_1hot, lr], outputs=None, updates=gen_param_updates)
...
for epoch in range(1200):
    ...
    if epoch==0:
        print(trainx.shape)
        #originally used 500 samples to init params, alternatively I can change gen network to flexible batch_size
        init_param(trainx[:100],trainy_1hot[:100]) # data based initialization
    ...
    for t in range(nr_batches_train):
        ...
        ll, lu, te = train_batch_disc(trainx[ran_from:ran_to],trainy[ran_from:ran_to],
                                      trainx_unl[ran_from:ran_to],trainy_1hot[ran_from:ran_to],lr)
        loss_lab += ll
        loss_unl += lu
        train_err += te
        
        train_batch_gen(trainx_unl2[t*args.batch_size:(t+1)*args.batch_size],trainy_1hot[t*args.batch_size:(t+1)*args.batch_size],lr)
    ...
    sample_x = samplefun(trainy_1hot[:100])
